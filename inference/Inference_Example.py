...
# Local utilities live one directory above this script.
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.append(str(PROJECT_ROOT))

# These imports need the sys.path modification above, so we keep them after
# appending PROJECT_ROOT and silence the lint warning about import position.
from deterministic_update import deterministic  # noqa: E402
from SFNO_update import SFNO  # noqa: E402
...
# Edit these defaults to hard-code your preferred settings when launching the
# script without CLI arguments (e.g., from a SLURM batch script).
DEFAULT_CONFIG = InferenceConfig(
    start="2019-03-22T00:00:00",
    steps=12,
    init_data=Path("/path/to/Initialize_2019_03_22T00_nsteps12.nc"),
    checkpoint_dir=Path("/path/to/checkpoints"),
    checkpoint_name="ckpt_mp0_epoch1.tar",
    output=Path("/path/to/output/forecast.nc"),
    variables=None,
    ema=False,
)
