...
#                   mse_value=None, contour_metrics=True):
#     """Helper to plot a single panel (contourf + optional contours + formatting)."""
    
#     # Initialize metrics variables to return
#     iou_ret = -1.0
#     amp_err_ret = float('inf')
...
#                 metrics_text.append(f"Amp: {amp_truth:.1f}")
...
#                 metrics_text.append(f"MSE: {mse_value:.2f}")
            
#             if mx is not None and tx is not None:
#                 # print('ckpt_num:', ckpt_num, 'mx, my, tx, ty', mx, my, tx, ty)
#                 # IoU
#                 iou = get_IoU_of_contours(mx, my, tx, ty)
#                 metrics_text.append(f"IoU: {iou:.2f}")
                
#                 # Amplitude Difference (Model Amp - Truth Amp)
#                 # contour_data is model data, truth_contour_data is truth data
#                 amp_model = get_amplitude_of_contour(c_data, mx, my)
#                 # metrics_text.append(f"Amp: {amp_model:.1f}")
...
#                 metrics_text.append(f"% err amp: {percent_error:.1f}")

#                 # Assign to return variables
#                 iou_ret = iou
#                 amp_err_ret = percent_error
...
#         info_str = "\n".join(metrics_text)
#         ax.text(0.97, 0.03, info_str, 
#                 transform=ax.transAxes, 
#                 color='black', fontsize=11, 
#                 bbox=dict(boxstyle='round', fc="w", ec="k", alpha=0.9),
#                 ha='right', va='bottom')
    
#     return iou_ret, amp_err_ret
        ...
        for panel in all_metrics_tracker:
            ...
            if 'mse' in panel and best_mse is not None:
                if panel['mse']['val'] == best_mse:
                    panel['mse']['text_obj'].set_bbox(dict(boxstyle='round', fc="w", ec="lime", alpha=0.9, lw=2.2))
            
            # Check IoU
            if 'iou' in panel and best_iou is not None:
                if panel['iou']['val'] == best_iou:
                    panel['iou']['text_obj'].set_bbox(dict(boxstyle='round', fc="w", ec="lime", alpha=0.9, lw=2.2))

            # Check % Err (using abs comparison)
            if 'err' in panel and best_err_abs is not None:
                if abs(panel['err']['val']) == best_err_abs:
                    panel['err']['text_obj'].set_bbox(dict(boxstyle='round', fc="w", ec="lime", alpha=0.9, lw=2.2))
...
#             # Store metrics for finding the best panels later
...
#     # Find best metrics and color borders green
#     if len(metrics_tracker) > 0:
#         best_mse_entry = min(metrics_tracker, key=lambda x: x['mse'])
#         best_iou_entry = max(metrics_tracker, key=lambda x: x['iou'])
#         best_amp_entry = min(metrics_tracker, key=lambda x: abs(x['amp_err']))

#         print('[best_mse_entry, best_iou_entry, best_amp_entry]', [best_mse_entry, best_iou_entry, best_amp_entry])
        
#         # Highlight best panels
#         for entry in [best_mse_entry, best_iou_entry, best_amp_entry]:
#             ax = entry['ax']
#             # make a box around the metric text e.g. if it's the best mse, then put a green box around the mse_w metric text 
#             for text in ax.texts:
#                 if 'MSE' in text.get_text() and entry == best_mse_entry:
#                     text.set_bbox(dict(boxstyle='round', fc="w", ec="lime", alpha=0.9, lw=2))
#                 if 'IoU' in text.get_text() and entry == best_iou_entry:
#                     text.set_bbox(dict(boxstyle='round', fc="w", ec="lime", alpha=0.9, lw=2))
#                 if '% err amp' in text.get_text() and entry == best_amp_entry:
#                     text.set_bbox(dict(boxstyle='round', fc="w", ec="lime", alpha=0.9, lw=2))
...
def mse_ckpt_leadtimes(ckpts, mses, var, experiment_number):
    ...
    for leadtime in [3,5,7]:
        ...
        ax.plot(odd_ckpts, mses_select, label=f'leadtime = {leadtime} days', color=colors[leadtime],)

    ax.set_xlabel('Checkpoint')
    # make the xlabel have finer ticks for ckpts
    ax.set_xticks(ckpts[::3])
